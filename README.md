...
>
> 주로 활용되는 쿼리문들이 모두 method로 구성 되어있다.

```bash
$ python manage.py shell
$ python manage.py shee_plus
```
...
* `shell_plus`는 `django_extentions`를 설치 후 `INSTALLED_APPS`에 등록하고 활용해야 한다.
...
* 특정(단일) 데이터 조회
  ...
  * 따라서, 단일 데이터 조회시(Primary key)에만 사용한다.
...
* 특정 데이터 조회
  ...
  Article.objects.filter(title__startswith='제목') # 제목응로 시작하는 제목
